...
def read_topics(dirname):
    ...
    for line in lines:
        ...
        project_details[id] = dict(
            ...
            Gruppeplacering=(((len(parts) > 6 and len(parts) == 12) and parts[11]) or (
                len(parts) > 6 and parts[10]) or "")  # to take into account format before 2012
        )
...
def check_sol():  # tablefile=''):
            ...
            if (groups[s1][0] == groups[s2][0]):
                if (ass[s1][0] != ass[s2][0] or ass[s1][1] != ass[s2][1]):
                    print(s1, " and ", s2, "not same group:",
                          ass[s1][0], ass[s1][1], ass[s2][0], ass[s2][1])
    ...
    for i in sorted(topics.keys()):
        for j in sorted(topics[i]):
            ...
            if (project_details[pID]["LedigePladser"] < 0):
                sys.exit('project %s has LedigePladser %s ' %
                         (pID, project_details[pID]["LedigePladser"]))
            ...
            if std_assigned > 0:
                for sID in sorted(assgrpid[pID]):
                    ...
                    if (project_details[pID]["Institutforkortelse"] == "IMADA"):
                        print(str(student_details[sID]["Navn"])+" "+student_details[sID]
                              ["Email"]+" "+str(student_details[sID]["Prioriteringsliste"]))
    ...
    for s in students:  # problem.groups.keys():
        ...
        valid_prjs = [x for x in sorted(topics.keys()) if project_details[str(
            x)+topics[x][0]]["MinProjektType"] in type_compatibility[peek]]
    ...
    for s in students:
        ...
        valgt = [x for x in range(1, len(priolist)+1) if int(priolist[x-1])
                 == int(project_details[pID]["ProjektNr"])]
...
def count_popularity():
    ...
    for i in sorted(topics.keys()):
        ...
        f.write(project_details[pID]["ProjektType"]+";" +
                project_details[pID]["Institutforkortelse"]+";")
...
def institute_wise():
    ...
    # print institutes
    pIDs_per_institute = {i: [x for x in pIDs if project_details[x]
                              ["Institut"] == i] for i in institutes}
    topics_per_institute = {i: [x for x in sorted(topics.keys()) if project_details[str(
        x)+topics[x][0]]["Institut"] == i] for i in institutes}
    ...
    for i in sorted(institutes):
        ...
        for pID in pIDs_per_institute[i]:
            std_assigned = pID in assgrpid and len(assgrpid[pID]) or 0
            if std_assigned > 0:
                s = istr+project_details[pID]["ProjektNrBB"] + \
                    ": "+project_details[pID]["ProjektTitle"]
                # # print "Popularity: (tot. "+str(popularity[i][0])+") "+str(popularity[i][1:(max_p+1)]);
                f.write(s+"\n")
                assigned = 0
                for sID in sorted(assgrpid[pID]):
                    s = str(student_details[sID]["Brugernavn"])+" " + \
                        str(student_details[sID]["Prioriteringsliste"])
                    # f.write(s+"\n")
                    assigned = assigned+1
    ...
    for s in student_details:
        ...
        for i in sorted(topics_per_institute.keys()):
            shared[i] = [v for v in topics_per_institute[i]
                         if v in student_details[s]["Prioriteringsliste"][:5]]
            lshared[i] = len(shared[i])
            # print s+" "+', '.join(map(str,shared[i]))+" "+i
            # print s+" "+str(len(shared[i]))+" "+i
        m = max([len(shared[x]) for x in shared])
        std_retning = student_details[s]["Studieretning"] if "Studieretning" in student_details[s] else filter(
    ...
    stds_per_retning = {
        f: len([s for s in student_details if student_details[s]["Studieretning"] == f]) for f in fields}
