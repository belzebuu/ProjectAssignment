...
def model_ip_weighted(prob, weight_method, instability, minimax, allsol):
    ...
    ############################################################
    # weighted
    m.addConstr(v >= quicksum(weights[grp_ranks[g][p]] * a[g] * x[g, p, t] for g in list(prob.groups.keys())
                              for p in list(grp_ranks[g].keys()) if p in prob.projects for t in range(len(prob.projects[p]))), 'v')
            ...
            for p in list(grp_ranks[g].keys()):
                for p2 in list(grp_ranks[g].keys()):
                    if (grp_ranks[g][p2] < grp_ranks[g][p]):
                        if p2 not in prob.projects or p not in prob.projects:
                            continue
                        for t in range(len(prob.projects[p])):
    ...
    if minimax >= 0:
        for g in cal_G:
            m.addConstr(u[g] ==
                        quicksum(grp_ranks[g][p] * x[g, p, t] for p in list(grp_ranks[g].keys()) if p in prob.projects
                                 for t in range(len(prob.projects[p]))),
                        'u_%s' % (g))
