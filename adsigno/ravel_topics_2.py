...
def main():
    ...
    dirname = pathlib.Path(args[0])

    problem = ld.Problem()

    topic_details = problem.read_topics(dirname)

    src=dirname / "projects.csv"
    dst=dirname / "projects.csv.bk"
...
def expand_topics(topic_details):
    OD=OrderedDict()
